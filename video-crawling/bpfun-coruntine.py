...
class BpfunVideoCrawler:
    def __init__(self, page_url: str, output_dir: str = r"D:\项目\crawler\py-playwright\video-crawling\output\bpfun2", max_workers: int = 10):
        """
        初始化视频爬虫
    ...
    async def get_m3u8_url_from_page(self, session: aiohttp.ClientSession) -> str:
        """
        从页面源码中提取m3u8地址
        ...
        async with session.get(self.page_url, headers=page_headers, timeout=aiohttp.ClientTimeout(total=30)) as response:
    ...
    async def fetch_m3u8_content(self, session: aiohttp.ClientSession, m3u8_url: str) -> str:
        """获取m3u8文件内容"""
        print(f"正在获取m3u8文件: {m3u8_url}")
        async with session.get(m3u8_url, headers=self.headers, timeout=aiohttp.ClientTimeout(total=30)) as response:
            return await response.text()
    
    async def parse_m3u8(self, session: aiohttp.ClientSession, m3u8_content: str, m3u8_url: str) -> tuple[list[str], float]:
        """
        ...
        if has_ts:
            ...
            for line in lines:
                if not line.startswith('#') and line.strip():
                    ...
                    # 获取子m3u8内容
                    sub_content = await self.fetch_m3u8_content(session, sub_m3u8_url)
                    # 递归解析子m3u8
                    ts_urls, segment_duration = await self.parse_m3u8(session, sub_content, sub_m3u8_url)
                    break
    ...
    async def download_ts_segment(self, session: aiohttp.ClientSession, ts_url: str, index: int, 
                                  semaphore: asyncio.Semaphore) -> tuple[int, bool, int]:
        """
        下载单个ts片段
        ...
        async with semaphore:
            try:
                ...
                async with session.get(ts_url, headers=self.headers, timeout=aiohttp.ClientTimeout(total=60)) as response:
    ...
    async def download_all_segments(self, session: aiohttp.ClientSession, ts_urls: list[str]) -> bool:
        """
        使用协程并发下载所有ts片段
        ...
        print(f"\n开始下载 {total_count} 个视频片段，使用 {self.max_workers} 个协程...")
        ...
        # 创建所有下载任务
        tasks = [
            self.download_ts_segment(session, url, idx, semaphore)
        ...
        print(f"  并发协程数: {self.max_workers}")
        print(f"  理论加速比: ~{self.max_workers}x (相比单协程)")
        print(f"{'='*60}")
    ...
    async def run_async(self, target_duration: float = 300.0):
        """
        异步运行爬虫
        ...
        async with aiohttp.ClientSession() as session:
            # 2. 从页面获取m3u8地址
            m3u8_url = await self.get_m3u8_url_from_page(session)
            
            # 3. 获取并解析m3u8文件
            m3u8_content = await self.fetch_m3u8_content(session, m3u8_url)
            ts_urls, segment_duration = await self.parse_m3u8(session, m3u8_content, m3u8_url)
            ...
            # 5. 协程并发下载片段
            if not await self.download_all_segments(session, ts_urls_to_download):
                print("警告: 部分片段下载失败")
    ...
    def run(self, target_duration: float = 300.0):
        ...
        Args:
            target_duration: 目标时长(秒)，默认5分钟(300秒)
        """
        asyncio.run(self.run_async(target_duration))


def main():
    # Bpfun视频页面URL
    page_url = "https://m.bpfun.com/play/31226-0-1.html"
    
    # 创建爬虫实例
    crawler = BpfunVideoCrawler(
        page_url=page_url,
        output_dir=r"D:\项目\crawler\py-playwright\video-crawling\output\bpfun2",  # 输出到video-crawling/output/bpfun2目录
        max_workers=10  # 使用10个协程并发下载
    )
