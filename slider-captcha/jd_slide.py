...
class JDSliderCaptcha:
    ...
    async def download_images(self) -> bool:
        ...
        try:
            # 获取主图和滑块图元素
            main_image = await self.page.query_selector("#main_img")
            slot_image = await self.page.query_selector("#slot_img")
    ...
    def recognize_captcha(self) -> int | None:
        ...
        try:
            ...
            distance = best_loc[0]
            adjusted_distance = int(distance * 275 / 290) + 12  # 增加12px补偿
    ...
    async def drag_slider(self, distance: int):
        """执行拖动操作"""
        # 获取滑块元素
        slider = await self.page.query_selector(".move-img")
        if not slider:
    ...
    async def solve(self, max_retries: int = 1) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 等待验证码加载
                await self.page.wait_for_selector("#main_img", timeout=5000)
...
async def jd_login_with_captcha(username: str, password: str):
    """
    京东带滑块验证的登录流程
    """
    async with async_playwright() as p:
        ...
        try:
            ...
            print("点击密码登录...")
            try:
                pwd_login = await page.wait_for_selector("#pwd-login", timeout=10000)
                if pwd_login:
                    await pwd_login.click()
                    await asyncio.sleep(2)
            except:
                print("未找到密码登录按钮，可能已经是密码登录模式")
            
            # 2. 输入用户名（模拟真人输入）
            print(f"输入用户名: {username}")
            username_field = await page.wait_for_selector("#loginname", timeout=10000)
            if username_field:
            ...
            # 3. 输入密码（模拟真人输入）
            print("输入密码...")
            password_field = await page.wait_for_selector("#nloginpwd", timeout=10000)
            if password_field:
            ...
            # 4. 点击登录
            print("点击登录按钮...")
            login_button = await page.wait_for_selector("#loginsubmit", timeout=10000)
            if login_button:
            ...
            # 5. 检查是否出现滑块验证
            print("检查是否出现滑块验证...")
            slider_element = await page.query_selector("#main_img")
...
async def main():
    ...
    # 从环境变量读取京东用户名和密码
    username = os.getenv("JD_USERNAME", "your_jd_username")
    password = os.getenv("JD_PASSWORD", "your_jd_password")
    
    if username == "your_jd_username" or password == "your_jd_password":
        print("警告：请先在.env文件中设置JD_USERNAME和JD_PASSWORD")
        print("当前使用默认值，登录可能会失败")
    
    print(f"使用账号: {username}")
    
    # 执行京东登录
    await jd_login_with_captcha(username, password)
